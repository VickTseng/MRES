...
function initial(){		
	...
	window.onload = function(){	//載入頁面時，將圖卡及字符配對好
		correctCouple();
		block62.obj.css("display","none"), block63.obj.css("display","none"), block64.obj.css("display","none");//隱藏轉換按鈕 
		$.post("../mathematic/myphp/equivalence_data/A_collect_data/equivalence_A_question.php", 
		...
		$.post("../mathematic/myphp/equivalence_data/A_collect_data/equivalence_A_member_card.php", 
...
function giveQuestion(){	//給予題目	
	...
	block67.value = 	block67.nume / block67.denom;
...
function check(){	//判斷對錯
	...
	if(block1.use == false && block2.use == false && block66.use == false){		//假如三個空格的值都填對了
		...
		$.post("../mathematic/myphp/equivalence_data/A_collect_data/equivalence_A_total_time.php", 
		{time: data_log.time, date:now.toString().substring(0,33), ques: switchSymbol(ques_index)});//儲存此題的完成時間
		data_log.time = 0; //將時間歸零		
		
		$.post("../mathematic/myphp/equivalence_data/A_collect_data/equivalence_A_operate_times.php", 
...
function singleCheck(){	//判斷單一空格的對錯
	...
	for(var i=0; i<3; i++){
		if(cloze_gra[i].empty == "false"){
			if(cloze_gra[i].value == block67.value){	//此格答對了
	...
	if(block1.value == block67.value && block2.value == block67.value && block66.value == block67.value && block1.empty == "false" 
	&& block2.empty == "false" && block66.empty == "false"){	//三格都正確填滿時，顯示最後確認鍵
...
var timer = setInterval(calTotalTime,1000);
//handleObj.disableRubber(); //判斷目前Rubber band區域是可用?
